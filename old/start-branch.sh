...
select_option() {
    ...
    cursor_to $((${#options[@]} + 3))  # +3 to account for the blank lines
    cursor_blink_on
...
# Clear screen to select type
clear

# Display branch type menu
...
echo  # Add a blank line before displaying the selected branch type
echo "You selected: $branch_type - $description"
echo  # Add a blank line after displaying the selected branch type
...
if [ $? -eq 0 ]; then
    ...
    echo
    echo "When you're done, create a pull request to merge this branch into 'development'."
    echo "After testing, create another pull request to merge into 'production'."
    echo
else
    echo
